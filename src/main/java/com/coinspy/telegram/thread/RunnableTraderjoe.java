...
@Component
public class RunnableTraderjoe extends RunnableComponents implements Runnable{
    ...
    @Autowired
    public RunnableTraderjoe(ExplorerController controller,
    ...
    @Override
    public void run() {
        var blockNumber = controller.getLastBlockNumber(Dex.TraderJoe);
        while (!Thread.currentThread().isInterrupted()){
            var tokens = controller.getLastPairCreated(Dex.TraderJoe, blockNumber);
            if (!tokens.isEmpty()) botExecuteTask(messageFactory, telegramBot, tokens, channelId, Dex.TraderJoe);
